    ...
    for (int z{}; z < Chunk::voxelCount; ++z)
        for (int y{}; y < Chunk::voxelCount; ++y)
            for (int x{}; x < Chunk::voxelCount; ++x)
            {
                ...
                if (sqrt((float)(x - Chunk::voxelCount / 2) * (x - Chunk::voxelCount / 2) + (y - Chunk::voxelCount / 2) * (y - Chunk::voxelCount / 2) + (z - Chunk::voxelCount / 2) * (z - Chunk::voxelCount / 2)) <= Chunk::voxelCount / 2) {
                    voxel.material = VoxelMaterial::GRASS;
                }
            }
}
...
class Minecraft : public SE::Application
{
public:
    ...
    Minecraft()
        : Application("Minecraft", 1920, 1080)
    {
        camera->speed = 40.0f;
            ...
            for (int x{}; x < ChunkManager::chunkCount; ++x)
                for (int y{}; y < ChunkManager::chunkCount; ++y)
                    for (int z{}; z < ChunkManager::chunkCount; ++z)
                    {
                        ...
                        renderer.draw(*chunk.getMesh(setActiosOnMaterial, chunkData), chunkManager.chunkIB, *shader.get(), *atlas.get(), SE::DrawMode::LINES);
                    }
